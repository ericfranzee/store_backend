...
//        ]);
    ...
    public function bookingUserMaster(array $bookings, $key = 'master', ?string $message = null, ?string $type = null): void
    {
        foreach ($bookings as $booking) {
            ...
            $data = [
                'id'     => $booking->id,
                'price'  => $booking->price,
                'type'   => $type ?? PushNotification::NEW_BOOKING
            ];
     ...
     * @return void
     */
    public function bookingSeller(array $bookings, ?string $message = null, ?array $type = null): void
    {
        foreach ($bookings as $booking) {
     ...
     * @return void
     */
    public function sendAllUpdateBooking(Booking $model, string $key, array $data): void
    {
        //master notify
        [$message, $replace, $firebaseToken, $ids] = $this->getAttributes($model, $model->master_id, $key, $data);

        $this->sendNotification($model, $firebaseToken, $message, $model->id, $replace, $ids);
