...
class BookingService extends CoreService
{
        ...
        try {
            $models = DB::transaction(function () use ($data, $calculate) {
                ...
                if (isset($data['coupon'])) {
                    $coupon = OrderHelper::couponPrice($data, $data['coupon'], $calculate['total_price'], $calculate['rate']);
                }

                if (isset($calculate['total_gift_cart_price']) && isset($data['gift_cart_id'])) {

                    $calculate['gift_cart_id'] = $data['gift_cart_id'];
                    $calculate['user_id']      = $data['user_id'];
                ...
                foreach ($data['data'] as $key => $item) {
                    ...
                    $calculateValue = @$calculate['items'][$key];
                    $serviceMaster  = @$calculateValue['service_master'];

                    if (empty($calculateValue) || $serviceMaster?->id !== $item['service_master_id']) {
                    ...
                    if (data_get($item, 'service_extras.0')) {
                        $extras = ServiceExtra::find($item['service_extras']);
                    }
                    ...
                    $item['coupon_price']      = $coupon > 0 ? $coupon / count($data['data']) : $coupon;
                    $item['extra_price']       = $extras->sum('price');
                    $item['gift_cart_price']   = @($calculateValue['gift_cart_price'] / $rate) ?? 0;
                    $item['total_price']       = @($calculateValue['total_price']     / $rate) ?? 0;
                    ...
                    $models[] = $model->fresh((new BookingRepository)->getWith());
                }
    ...
    public function statusUpdate(int $id, array $filter): Booking
    {
        ...
        $model = Booking::with([
            'shop:id,b_count,b_sum',
            'master:id,b_count,b_sum',
            'user:id,b_count,b_sum',
        ])
            ->has('master')
            ->has('user')
            ->has('shop')
    ...
    public function notesUpdate(int $id, array $data): Booking
    {
        ...
        return DB::transaction(function () use ($model, $data) {
            ...
            $this->sendAllUpdateBooking($model, ResponseError::BOOKING_NOTE_UPDATED, $data);
    ...
    private function updateGiftCartPrice(array $data): float
    {
        $giftCart = UserGiftCart::where('user_id', $data['user_id'])
            ->where('gift_cart_id', $data['gift_cart_id'])
            ->first();
    ...
    public function checkAssignedBeforeUpdate(Booking $model): void
    {
        ...
        if ($user->hasRole('master') && $model->master_id !== $user->id) {

            throw new Exception(__('errors.' . ResponseError::ERROR_404, locale: $this->language));

        } else if ($user->hasRole('seller') && $model->shop?->user_id !== $user->id) {
