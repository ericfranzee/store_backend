...
class BookingActivityService extends CoreService
{
     ...
     * @return void
     */
    public function create(Booking $booking, string $type, string $locale, ?array $data = []): void
    {
        /** @var User $user */
        ...
        switch ($type) {
            ...
            case 'update':
                ...
                if ( // reschedule
                    isset($data['start_date']) && $booking->start_date !== $data['start_date'] ||
                    isset($data['end_date'])   && $booking->end_date   !== $data['end_date']
                ) {
