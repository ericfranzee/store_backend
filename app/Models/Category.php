    ...
    public function scopeFilter($value, $filter)
    {
        ...
        return $value
            ...
            ->when(data_get($filter, 'has_service'), function ($q) use ($filter) {
                return $q
                    ->when(isset($filter['shop_id']), function ($q) use ($filter) {
                        $q->whereHas('services', fn($q) => $q->where('shop_id', $filter['shop_id']));
                    })
                    ->whereHas('services.serviceMasters', fn($q) => $q->where('active', true))
                    ->has('services.serviceMasters.master.workingDays')
                    ->whereHas('services.serviceMasters.master.invitations', fn($q) => $q->where('status', 2));
            })
            ->when(data_get($filter, 'master_id'), function ($query, $masterId) {
                $query->whereHas('services.serviceMasters', fn($q) => $q->where('master_id', $masterId));
            })
            ->when(
