    ...
    public function scopeFilter(Builder $query, array $filter)
    {
        ...
        $query
            ...
            ->when(data_get($filter, 'service_prices'), function (Builder $q, $prices) {
                ...
                $q
                    ->where('service_min_price', '>=', $from)
                    ->where('service_max_price', '<=', $to);
            })
            ...
            ->when(data_get($filter, 'service_type'), function ($q, $type) {
                $q->whereHas('services', fn($q) => $q->where('type', $type));
            })
...
//                    })
            ...
            ->when(data_get($filter, 'order_by'), function (Builder $query, $orderBy) {
                ...
                if ($column !== 'distance') {
                    ...
                    if (in_array($column, ['r_count', 'r_avg', 'r_sum', 'o_count', 'o_sum', 'b_count', 'b_sum'])) {
                        $q->orderBy('id', $filter['sort'] ?? 'desc');
                    }
